...
void RangeUpdate(int ind,int low,int high,int l,int r,int val,vector<int>&seg,vector<int>&lazy){
    if(lazy[ind]!=0){ // update first if have lazy value
    ...
    // complete-overlap
    if(low>=l && high<=r){
        seg[ind]+=val*(high-low+1);  // high-low+1 se multiply isliye kyunki high-low+1 child h to sabki add hogi
...
int main()
{
   ...
   RangeUpdate(0,0,n-1,2,4,5,seg,lazy);
